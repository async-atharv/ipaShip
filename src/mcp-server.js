...
//  PROVENANCE_FINGERPRINT: ipaSh1p:src:mcp-server:c9f3a7e1b2d4
// ─────────────────────────────────────────────────────────────────────────────
...
function resolveConfig(projectDir) {
  ...
  if (!defaults) {
    throw new Error(`Unknown provider "${provider}". Set IPASHIP_PROVIDER to "gemini" or "claude".`);
  }

  const model = process.env.IPASHIP_MODEL || defaults.model;
